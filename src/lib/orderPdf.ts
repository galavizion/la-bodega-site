...
export interface OrderPdfData {
  ...
  customer?: {
    ...
    address?: string; colonia?: string; city?: string; state?: string; zip?: string; reference?: string;
  };
  items?: { variantLabel?: string; quantity?: number; unitPrice?: number }[];
  subtotal?: number;
  shipping?: number;
  iva?: number;
  total?: number;
...
export function buildOrderPdf(doc: OrderPdfData): jsPDF {
  const pdf = new jsPDF();
  ...
  sumY += 6;
  if ((doc.iva ?? 0) > 0) {
    pdf.text(`IVA: ${fmt(doc.iva ?? 0)}`, 140, sumY);
    sumY += 6;
  }
  if ((doc.cashbackApplied ?? 0) > 0) {
    pdf.text(`Cashback aplicado: -${fmt(doc.cashbackApplied ?? 0)}`, 140, sumY);
    sumY += 6;
  }
  pdf.setFontSize(11);
